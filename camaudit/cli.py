...
from .discovery import scan_network, onvif_discover, DEFAULT_CAMERA_PORTS
...
def cmd_scan(args):
    ...
    if args.dry_run:
        print(f"[DRY-RUN] Escanearía la red {args.network} en puertos "
              f"{args.ports or DEFAULT_CAMERA_PORTS}. No se envía tráfico real.")
        return
    ...
    if args.onvif:
        onvif_devices = onvif_discover(timeout=args.timeout)
        print(f"Dispositivos ONVIF detectados por WS-Discovery: {len(onvif_devices)}")

    findings = []
    for dev in devices:
        result = check_device(dev["ip"], dev["port"], timeout=args.timeout)
        if result:
            findings.append(result)
            print(
                f"{Fore.RED}[VULNERABLE]{Style.RESET_ALL} {dev['ip']}:{dev['port']} "
    ...
    print(
        f"\nResumen: {len(report['vulnerable_devices'])} dispositivo(s) con "
        f"credenciales por defecto de {len(devices)} puerto(s) abiertos revisados."
    )
...
def main():
    ...
    scan_parser.add_argument("--onvif", action="store_true", help="Además hacer descubrimiento ONVIF (WS-Discovery)")
